    ...
    def move(self):
        ...
        for m in range(self.i - 1, self.i + 2):
            for n in range(self.j - 1, self.j + 2):
                if (0 <= m < 8) and (0 <= n < 8) and not (
                        m == self.i and n == self.j):  # if out of board or at its place
                    king.append((m, n))
...
class Soldier:
    ...
    numberofsoldierlelf_black = 0
    numberofsoldierlelf_white = 0
    ...
    '''def end(self):
        if self.i == 7 or self.i == 0:
            if self.user:
                while True:
                    p = input('enter q: for queen
                                    h: for horse
                                    b: for bishop
                                    r: for rook')
                    if p in ['q','h','r','b']:
                        break
                if p == 'q':
                    self = Queen(self.i, self.j, self.user) # change type of pawn to queen
                    if self.user:
                        del black_pawns[board[self.i][self.j]] # remove soldier element from black_pawns
                        board[self.i][self.j] = 'bq' + str(numberofsoldierlelf_black + 8) # set the board with new pawn
                        black_pawns[board[self.i][self.j]] = self # update black_pawns with new element'''
...
images = {'WS' : cv2.imread('whitesoldier.jpg'), 'WR' : cv2.imread('whiterook.jpg'), 'WH' : cv2.imread('whitehorse.jpg'),'WB' : cv2.imread('whitebishp.jpg'),'WK' : cv2.imread('whiteking.jpg'), 'WQ' : cv2.imread('whitequeen.jpg'), 'bs' : cv2.imread('blacksoldier.jpg'),'br' : cv2.imread('blackrook.jpg'),'bh' : cv2.imread('blackhorse.jpg'),'bb' : cv2.imread('blackbishp.jpg'),'bk' : cv2.imread('blackking.jpg'),'bq' : cv2.imread('blackqueen.jpg')}

def print_board():
    b = cv2.imread('chess.jpg')
    for i in range(8):
        for j in range(8):
            if board[i][j] != '   ':
                pic = images[board[i][j][:-1]]
                x1 = 90*(i+1) + 10
                y1 = 90*(j+1) + 10
                x2 = 90 * (i+2) - 10
                y2 = 90 * (j+2) - 10
